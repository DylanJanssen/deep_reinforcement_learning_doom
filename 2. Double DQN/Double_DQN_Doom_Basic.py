...
model_savefile = "./model-doom-basic.pth"
training_rewards_savefile = "./training_rewards-basic.csv"
skip_learning = False
load_model = True
...
class ConvolutionalDeepQNet(nn.Module):
    def __init__(self, input_shape, num_actions):
        super(ConvolutionalDeepQNet, self).__init__() 
    ...
    def forward(self, x):
        ...
        x = self.fc(x)              # feed forward through fully connected layers 
        return x 
...
def compute_td_loss(batch_size):
    ...
    next_q_values = current_model(next_state_mb)
    next_q_state_values = target_model(next_state_mb)
...
if __name__ == '__main__':
    ...
    current_model = ConvolutionalDeepQNet(state.shape, len(actions))
    target_model  = ConvolutionalDeepQNet(state.shape, len(actions))
